  ...
  class ContentKey < Model::Base
    ...
    class << self
      def create_open_aes(request)
        ...
        ckap.link_options(ckapo)
        protection_key_id = self.get_protection_key_id(request)
        protection_key = self.get_protection_key(request, protection_key_id)
      ...
      def get_protection_key_id(request, content_key_type=4)
        request.get("GetProtectionKeyId", { contentKeyType: content_key_type })['value']
      end

      def get_protection_key(request, protection_key_id)
        request.get("GetProtectionKey", { ProtectionKeyId: "'#{protection_key_id}'" })['value']
      end

    end

    def add_authorization_policy(policy_id)
      @request.put("ContentKeys('#{CGI.escape(self.Id)}')", {AuthorizationPolicyId: policy_id})
    end
